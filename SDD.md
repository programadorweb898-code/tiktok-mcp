...
Agente MCP (Claude Code, Codex, Hermes, …)
        ↓  stdio (JSON-RPC)
src/server.ts            — definición y validación de las 32 tools MCP
        ↓
...
| `src/server.ts` | Registro de las 32 tools con esquemas zod, wrapper `addTool` con captura de errores y devolución estructurada (`structuredContent` + texto JSON). |
...
Verificado contra `src/server.ts`. El servidor expone exactamente 32 tools (el test lo afirma).
...
| analíticas de perfil (seguidores, totales) | RESEARCH | — |
| analíticas profundas de Studio (watch time, tráfico) | RESEARCH | Mencionado en código como follow-up |
...
| 3 | Discovery y trends: búsquedas (videos/usuarios/hashtags), tendencias, sounds | En curso (búsqueda implementada `tiktok_search`; sounds `tiktok_sounds`; tendencias/topics `tiktok_trending_topics`; discover/FYP `tiktok_trending`; creadores pendiente) |
