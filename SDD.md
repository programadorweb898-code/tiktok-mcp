...
Agente MCP (Claude Code, Codex, Hermes, …)
        ↓  stdio (JSON-RPC)
src/server.ts            — definición y validación de las 38 tools MCP
        ↓
...
Verificado contra `src/server.ts`. El servidor expone exactamente 38 tools (el test lo afirma).
...
| photo (carrusel) | PARTIAL (v1) | `tiktok_photo_post` publica 1-35 imágenes como carrusel vía Studio (caption, scheduling nativo, privacidad, toggles y verificación como `tiktok_post`). La superficie de edición del carrusel (paso intermedio entre upload y caption) no se inspeccionó en desarrollo → avance por texto visible, validación manual pendiente |
| draft | RESEARCH | — |
...
## DEC-024 — Fase 6: Photo posts (`tiktok_photo_post`)

Fecha: 2026-08-31

Estado: Aceptada (implementada v1, validación manual pendiente)
Decisión: implementar `tiktok_photo_post`, que publica (o programa con el scheduler nativo) un carrusel de 1..35 imágenes vía Studio, aceptando `image_path`/`image_url`/`image_base64` por imagen y preservando el orden. Reutiliza el flujo de `postVideo` (materializar media, subir vía `setInputFiles`, caption con resolución resiliente, schedule/privacy/duet/stitch, submit + intercept de API + fallback por redirect y verificación por caption) y `materializeImage` (loop que materializa N imágenes y las limpia en `finally`).
