...
Agente MCP (Claude Code, Codex, Hermes, …)
        ↓  stdio (JSON-RPC)
src/server.ts            — definición y validación de las 33 tools MCP
        ↓
...
| `src/server.ts` | Registro de las 33 tools con esquemas zod, wrapper `addTool` con captura de errores y devolución estructurada (`structuredContent` + texto JSON). |
...
Verificado contra `src/server.ts`. El servidor expone exactamente 33 tools (el test lo afirma).
